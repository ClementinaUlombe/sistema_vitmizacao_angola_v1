...
export async function sendEmail({ to, subject, html }: SendEmailOptions) {
  try {
    const info = await transporter.sendMail({
      from: `"Safe Angola - Vitimização Criminal" <${process.env.SMTP_USER}>`,
      to,
      subject,
      html,
    });
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
      <div style="background-color: #1e3a8a; color: white; padding: 20px; text-align: center;">
        <h1 style="margin: 0;">Bem-vindo ao Safe Angola</h1>
      </div>
      <div style="padding: 30px; line-height: 1.6; color: #333;">
        <p>Olá, <strong>${name}</strong>,</p>
        <p>A sua conta no sistema de <strong>Vitimização Criminal</strong> foi criada com sucesso!</p>
        <p>Abaixo estão os detalhes do seu acesso:</p>
        <ul style="list-style: none; padding: 0;">
          <li><strong>Perfil:</strong> ${roleName}</li>
        </ul>
